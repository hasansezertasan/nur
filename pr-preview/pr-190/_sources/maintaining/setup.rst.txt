...
`Contributing Guidelines <https://github.com/hasansezertasan/nur/blob/main/.github/CONTRIBUTING.md>`_.
...
.. code-block:: sh
   ...
   gh api repos/hasansezertasan/nur \
...
.. code-block:: sh
   ...
   gh api repos/hasansezertasan/nur/branches/main/protection \
...
.. code-block:: sh
   ...
   gh api repos/hasansezertasan/nur/actions/permissions/workflow \
...
.. code-block:: sh
   ...
   gh api --silent \
     repos/hasansezertasan/nur/dependency-graph/compare/main...main
...
.. code-block:: sh
   ...
   gh api -X PUT repos/hasansezertasan/nur/environments/publish \
     -F 'deployment_branch_policy[protected_branches]=false' \
     -F 'deployment_branch_policy[custom_branch_policies]=true'
   ids="$(gh api --paginate repos/hasansezertasan/nur/environments/publish/deployment-branch-policies \
     --jq '.branch_policies[].id')"
   for id in $ids; do
     gh api -X DELETE "repos/hasansezertasan/nur/environments/publish/deployment-branch-policies/$id"
   done
   gh api -X POST repos/hasansezertasan/nur/environments/publish/deployment-branch-policies \
...
.. code-block:: sh
   ...
   gh variable set ENABLE_PRIVATE_ATTESTATIONS --body true --repo hasansezertasan/nur
...
.. code-block:: sh
   ...
   policies="$(gh api --paginate repos/hasansezertasan/nur/environments/publish/deployment-branch-policies \
...
.. code-block:: sh
   ...
   priv="$(gh repo view hasansezertasan/nur --json isPrivate --jq '.isPrivate' 2>/dev/null)"
   if [ "$priv" = "false" ]; then true
   elif [ "$priv" = "true" ]; then
     gh secret list --repo hasansezertasan/nur --json name --jq '.[].name' | grep -qx CODECOV_TOKEN
   else false; fi
...
.. code-block:: sh
   ...
   gh secret set GITLEAKS_LICENSE --repo hasansezertasan/nur
...
.. code-block:: sh
   ...
   owner="$(gh api repos/hasansezertasan/nur --jq '.owner.type' 2>/dev/null)"
   if [ "$owner" = "User" ]; then true
   elif [ "$owner" = "Organization" ]; then
     gh secret list --repo hasansezertasan/nur --json name --jq '.[].name' | grep -qx GITLEAKS_LICENSE
   else false; fi
...
.. code-block:: sh
   ...
   gh api repos/hasansezertasan/nur/pages \
...
.. code-block:: sh
   ...
   [ "$(gh repo view hasansezertasan/nur --json hasDiscussionsEnabled --jq '.hasDiscussionsEnabled')" = "true" ]
...
.. code-block:: sh
   ...
   gh secret list --repo hasansezertasan/nur --json name \
...
``https://repology.org/project/nur/versions`` and verify that
...
``nur``. If it does, use the name shown by Repology in both
URLs below.
...
.. code-block:: markdown
   ...
   [![Packaging status](https://repology.org/badge/vertical-allrepos/nur.svg)](https://repology.org/project/nur/versions)
