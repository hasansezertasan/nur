...
not consumed by nur itself. Copy its contents into your
...
3. Bootstrap the initial formula:
   ...
   brew create --set-name nur <pypi-sdist-url> --tap hasansezertasan/tap
   brew update-python-resources hasansezertasan/tap/nur
   ```

   These write `Formula/nur.rb` into brew's own checkout of the
...
5. Push everything so the tap repository actually contains the listener and
   ...
   ```sh
   git add -A
   git commit -m "Add nur + release listener"
   git push
   ```

## Token setup (in this project's repo)

In **this** project's repository — `hasansezertasan/nur` — add
